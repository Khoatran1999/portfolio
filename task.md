...
scrub.vars.offset = (iteration + self.progress) _ seamlessLoop.duration();
...
progressToScroll = progress => gsap.utils.clamp(1, trigger.end - 1, gsap.utils.wrap(0, 1, progress) _ trigger.end),
...
startTime = items.length _ spacing + 0.5, // the time on the rawSequence at which we'll start the seamless loop
loopTime = (items.length + overlap) _ spacing + 1, // the spot at the end where we loop back to the startTime
